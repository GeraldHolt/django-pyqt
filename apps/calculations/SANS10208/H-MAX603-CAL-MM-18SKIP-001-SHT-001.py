...
def main():
    ...
    with doc.create(Section(SECTION_4)) as sec:                
        with sec.create(Subsection(SECTION_4_1)) as subsec2:
            with subsec2.create(Subsubsection(SECTION_4_1_1)) as subsubsec1:
                d = general_skip_loads()
                with subsubsec1.create(FlushLeft()) as left:
                    with left.create(MiniPage(align='l')) as tab:
            ...
            with subsec2.create(Subsubsection(SECTION_4_1_2)) as subsubsec2:
                d = permanent_loads()
                ...
                with subsubsec2.create(Figure(position='h!')) as fig:
                    fig.append(StandAloneGraphic(
            ...
            with subsec2.create(Subsubsection(SECTION_4_1_7)) as subsubsec5:
                d = emergency_loads()
                with subsubsec5.create(FlushLeft()) as left:
                    with left.create(MiniPage(align='l')) as tab:
                        with tab.create(Tabular('l l l l')) as table:
            ...
            with subsec2.create(Subsubsection(SECTION_4_1_13)) as subsubsec6:
                d = rock_loads()
                with subsubsec6.create(FlushLeft()) as left:
                    with left.create(MiniPage(align='l')) as tab:
                        with tab.create(Tabular('l l l l')) as table:
                            for k,v in d.items():
                                descrip = v[0]
        ...
        with sec.create(Subsection(SECTION_4_3)) as subsec4:
            with subsec4.create(Subsubsection(SECTION_4_3_1)) as subsubsec7:
                with subsubsec7.create(Figure(position='h!')) as fig:
                    fig.append(StandAloneGraphic(
                            image_options="width=360px",
                            filename=fix_filename(SKETCH_9[0])
                            ))
                    fig.append(Command('centering'))
